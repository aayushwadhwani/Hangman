...
print(os.getcwd())
# to check if won or lost? result !
def wonorlost(message):
    if message =="You Won! ":
        ...
        pygame.time.delay(1000)
        text = wordfont.render(f"{message}",1,black)
        wordwas = wordfont.render(f"The Word was :{word}",1,black)
        WIN.fill(white)
        WIN.blit(text,(width/2-text.get_width()/2,height/2-text.get_height()/2))
        WIN.blit(wordwas,(width/2-text.get_width()/2-75,height/2+30-text.get_height()/2))
        pygame.display.update()
        pygame.time.delay(2000)        
...
def draw():
    ...
    display = ""
    WIN.fill(white)
    chances = chancesfont.render(f"Chances left: {6-hangman_status}",1,black)
    WIN.blit(chances,(0,0))
    ...
    for letter in word:
        if letter in gussed:
            ...
            display += f"_ "
    worddisplay = wordfont.render(display,1,black)
    WIN.blit(worddisplay,(350,175))
    WIN.blit(images[hangman_status],(75,100))
    for letter in letters:
        xcord,ycord,ltr, isvisible = letter
        if isvisible:
            pygame.draw.circle(WIN, black, (xcord,ycord), radius, 3 )
            text = font.render(ltr, 1, black)
            WIN.blit(text, (xcord-text.get_width()/2,ycord-text.get_height()/2))
    pygame.display.update()
...
for loop in range (26):
    x = startx + gap*2 + ((radius*2+gap)*(loop%13))
    y = starty + ((loop//13)*(gap+radius*2))
    letters.append([x,y,chr(65+loop), True])

# font
font = pygame.font.SysFont('lucidasans', 30)
wordfont = pygame.font.SysFont('lucidasans', 30)
titlefont = pygame.font.SysFont('lucidasans', 40)
chancesfont = pygame.font.SysFont('lucidasans', 20)
